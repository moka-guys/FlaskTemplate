...
logger = logging.getLogger("{{cookiecutter.project_name}}_logger")
...
app.config["UPLOAD_EXTENSIONS"] = [
    ".{{cookiecutter.single_file_allowable_extension}}",
    ".{{cookiecutter.multi_file_allowable_extension}}",
...
app.config["APPLICATION_ROOT"] = "/{{cookiecutter.project_name}}"
...
webapp_bp = Blueprint(
    ...
    __name__,
    url_prefix="/{{cookiecutter.project_name}}",
)
Session(app)
...
def validate_input_file(form, field):
    ...
    if not (
        ...
        raise ValidationError(
            f"Invalid file type for input file '{file.filename}'. Allowed types are: {{cookiecutter.single_file_allowable_extension}}"  # TODO: Ammend file types as appropriate i.e. "xlsm, xlsx"
        )
...
def validate_input_files_array(form, field):
    ...
    for file in files:
        ...
        if not (
            ...
            raise ValidationError(
                f"Invalid file type for input files '{file.filename}'. Allowed types are: {{cookiecutter.multi_file_allowable_extension}}"  # TODO: Ammend file types as appropriate i.e. "xlsm, xlsx"
            )
...
class ChangeForm(FlaskForm):
    single_file = FileField(
        ...
        render_kw={
            "accept": ".{{cookiecutter.single_file_allowable_extension}}",
            "id": "single_file",
        },  # TODO: If multiple file types, add to list i.e. [".xlsm, .xlsx"]
    )
    multiple_array_of_files = MultipleFileField(
        ...
        render_kw={
            "accept": ".{{cookiecutter.multi_file_allowable_extension}}",
            "multiple": "True",
            "id": "multiple_array_of_files",
        },  # TODO: If multiple file types, add to list i.e. [".xlsm, .xlsx"]
    )
    submit = SubmitField("Run {{cookiecutter.project_name}}")
...
def form(backend_state="initial"):
    ...
    if request.method == "POST" and chgForm.validate_on_submit():
        ...
        file_handler = logging.FileHandler(
            f"/var/local/{{cookiecutter.project_name}}/logs/{{cookiecutter.project_name}}_error.log"
        )
